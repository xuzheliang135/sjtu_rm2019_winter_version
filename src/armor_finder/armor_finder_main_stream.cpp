        ...
        case SEARCHING_TARGET:
            ...
            if (target_found_frame_cnt > state_machine_param_.THRESHOLD_FOUND_SEARCHING_TO_TRACKING) {
                ...
                if ((Rect2d(0, 0, 640, 480) & armor_box_on_raw_left_).area() < armor_box_on_raw_left_.area() ||
                    (Rect2d(0, 0, 640, 480) & armor_box_on_raw_right_).area() <
                    armor_box_on_raw_right_.area()) { // avoid box touching edges
                    target_found_frame_cnt = 0;
                    break;
                }
                Mat roi_left = src_raw_left_.clone()(armor_box_on_raw_left_);
                Mat roi_right = src_raw_right_.clone()(armor_box_on_raw_right_);
