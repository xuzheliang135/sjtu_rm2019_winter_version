...
void ArmorFinder::initCalibrateParam() {
    std::string calibrate_intrinsic_filename = "../extra_files/camera_calibration_parameter/intrinsics.yml";
    std::string calibrate_extrinsic_filename = "../extra_files/camera_calibration_parameter/extrinsics.yml";
    ...
    fs_intrinsic["D1"]>>D1;
    fs_intrinsic["M2"]>>M2;
    fs_intrinsic["D2"]>>D2;  //读取内参数矩阵文件中的矩阵

    Mat R, T, R1, P1, R2, P2;
    fs_extrinsic["R"]>>R;
    fs_extrinsic["T"]>>T;    //读取外参数矩阵文件中的矩阵

    stereoRectify(M1, D1, M2, D2, image_size, R, T, R1, R2, P1, P2, Q, CALIB_ZERO_DISPARITY, -1, image_size, &roi1, &roi2);
